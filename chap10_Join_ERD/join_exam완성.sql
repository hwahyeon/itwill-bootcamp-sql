...
user_id int not null,  -- 고객id(외래키)          
return_code number(1) not null,  -- 반품사유(1~4)
foreign key(user_id) 
references User_data(user_id)    
);
...
-- 조건2) 고객ID 오름차순 정렬
select u.user_id, u.gender, u.age, u.job, 
p.product_type, p.pay_method, p.price 
from user_data u, pay_data p
where u.user_id = p.user_id -- join 조건 
order by u.user_id; -- 고객id 오름차순 정렬
-- [해설] 전체 고객 중에서 상품 구매 고객 정보 조회 
...
where u.user_id = p.user_id and
 (u.gender=2 or p.pay_method='1.현금')-- join 조건 
order by u.user_id; 
-- [해설] 상품구매 고객 중에서 특정 고객 조회
...
-- 조건2) 반품한 고객만 출력
select u.user_id, u.gender, u.age, u.resident,
r.return_code 
from user_data u, return_data r
where u.user_id = r.user_id(+) 
and r.return_code is not null-- join 조건 
order by u.user_id;
-- [해설] 전체 고객 중에서 상품을 반품한 고객 조회 
